...
type SpaceNavigation = { onOpenSpace: (id: string) => void }

export function Hero({ onOpenSpace }: SpaceNavigation) {
  return <section className="hero" aria-labelledby="greeting"><p className="eyebrow">YOUR SPACE. YOUR PACE.</p><h1 id="greeting">下午好，今天想做什么？</h1><p className="hero-subtitle">专注当下 · 构建更好的自己</p><SearchTrigger /><SpaceChips onOpenSpace={onOpenSpace} /></section>
}
...
export function BentoGrid({ onOpenSpace }: SpaceNavigation) {
  return <section className="spaces-section" aria-labelledby="spaces-heading"><div className="section-label"><h2 id="spaces-heading">我的空间 <span>MY SPACES</span></h2><span>让每一个入口，都有归处</span></div><div className="bento-grid">{spaces.map(space => {
    ...
    return <article key={space.id} onClick={event => { if (!(event.target as Element).closest('a, button')) onOpenSpace(space.id) }} className={`space-card space-${space.id} ${active ? 'expanded' : 'compact'}`}><div className="space-card-heading"><span className="space-icon"><Icon name={space.icon} /></span><div><h3><button type="button" className="space-card-entry" aria-label={`进入${space.name}`} onClick={() => onOpenSpace(space.id)}>{space.name}</button></h3><p>{space.subtitle}</p></div>{active && <span className="active-indicator" aria-label="当前空间" />}</div>{items.length ? <div className="resource-grid">{items.slice(0, active ? 8 : 3).map(resource => active ? <ResourceItem key={resource.id} resource={resource} /> : <ResourceLink key={resource.id} resource={resource} />)}</div> : <div className="space-empty"><span className="empty-line" /><span>尚未添加资源</span><span className="empty-line" /></div>}<div className="space-card-footer"><span>{items.length ? `${items.length} 个资源` : '待补充'}</span><span className="current-label">浏览空间 <span>↗</span></span></div></article>
  })}</div></section>
}
