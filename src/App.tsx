...
import { ContinueProvider, ContinueSection } from './components/Continue'
...
import { Hero, BentoGrid } from './components/Home'
...
export function App() {
  ...
  return <FavoritesProvider><RecentProvider><ContinueProvider><PaletteProvider>
    <div className="app-shell" id="home">
      ...
      <div className="app-body"><TopBar /><main id="main" ref={mainRef} tabIndex={-1}>
        {view.kind === 'home' ? <><Hero onOpenSpace={openSpace} /><BentoGrid onOpenSpace={openSpace} /><div className="activity-stack"><ContinueSection /><RecentResources /></div></> : view.kind === 'favorites' ? <FavoritesView /> : view.kind === 'workspace' ? <WorkspaceView /> : <SpaceView spaceId={view.spaceId} onOpenSpace={openSpace} onHome={() => navigate({ kind: 'home' })} />}
        <Footer />
      </main></div>
    </div>
  </PaletteProvider></ContinueProvider></RecentProvider></FavoritesProvider>
}
